...
**기본적으로 Vercel에 배포된 모든 사이트는 Public입니다!**

- 배포된 URL을 아는 사람은 누구나 접근 가능
- 별도의 인증이나 로그인 불필요
- 검색 엔진에 노출 가능 (robots.txt 설정 가능)

### 현재 배포된 URL
- **Production URL:** https://baseball-hazel.vercel.app
- 이 URL을 공유하면 누구나 접근 가능합니다
...
2. **검색 엔진 최적화 (SEO)**
   - `public/robots.txt` 파일 추가 가능
   - `index.html`의 meta 태그 확인
